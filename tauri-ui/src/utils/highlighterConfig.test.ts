...
describe('highlighterConfig', () => {
  it('has all expected languages', () => {
    expect(Object.keys(highlighterConfig)).toEqual(
      expect.arrayContaining(['python', 'tsx', 'javascript', 'html', 'css'])
    );
  });
...
`;
    ...
    expect(
      code.match(rules.find(r => r.name === 'comment')!.pattern)
    ).toContain('# comment');
    // Should match string
    expect(code.match(rules.find(r => r.name === 'string')!.pattern)).toContain(
      '"hello"'
    );
    // Should match number
    expect(code.match(rules.find(r => r.name === 'number')!.pattern)).toContain(
      '123'
    );
    // Should match keyword
    expect(
      code.match(rules.find(r => r.name === 'keyword')!.pattern)
    ).toContain('def');
    // Should match builtin
    expect(
      code.match(rules.find(r => r.name === 'builtin')!.pattern)
    ).toContain('print');
    // Should match method (should NOT match here)
    expect(
      code.match(rules.find(r => r.name === 'method')!.pattern)
    ).toBeNull();
  });
...
/* multi
   line */
    ...
    // Method
    expect(code.match(rules.find(r => r.name === 'method')!.pattern)).toContain(
      '.log'
    );
    // Comment
    expect(
  ...
  it('html rules match code', () => {
    ...
    expect(
      code.match(rules.find(r => r.name === 'string')!.pattern)
    ).not.toBeNull();
    expect(
      code.match(rules.find(r => r.name === 'builtin')!.pattern)
    ).not.toBeNull();
